...
**核心理念**：不再每个 material 绑定一个 descriptor set。所有纹理、采样器、Buffer 注册到一个全局资源表中，shader 通过整数索引访问。draw call 间无需切换 descriptor set——只 push 索引常量。

#### 为什么用 Bindless

| 传统方式 | Bindless |
|---|---|
| 每个 material 一个 VkDescriptorSet | 全场景共享 1-3 个 VkDescriptorSet |
| Draw 前 `vkCmdBindDescriptorSets` | Draw 前 `vkCmdPushConstants`（传几个 uint32 index） |
| 材质切换 = descriptor set 切换（开销大） | 材质切换 = push 不同的索引（几乎零开销） |
| GPU culling 困难——需要 CPU 先绑定再 draw | GPU culling 友好——shader 拿到 index 就能访问任何资源 |
| RenderResource 需持有 `RHIDescriptorSet` | RenderResource 只持有 `uint32_t m_bindlessIndex` |

#### Vulkan 机制

依赖 Vulkan 1.2+ 的 `VK_EXT_descriptor_indexing`（promoted to core）：

- **`VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT`** — 允许在 command buffer 录制期间更新 descriptor set（而非仅在 submit 前）。关键：descriptor set 绑定后，写入才可见。
- **`VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT`** — shader 可声明 `texture2D g_Textures[65536]` 但只有前 N 个有效 slot，访问越界由 robustBufferAccess 处理。
- **`VK_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT`** — 池支持 after-bind 更新（已设置 ✅）。
- **`VK_DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT_BIT`** — binding 实际大小由 `vkAllocateDescriptorSets`/`vkCmdBindDescriptorSets` 时的 count 决定。

#### 全局 Descriptor Set 布局

参考 UE5 和 ToyRenderer，分三层 set：

```
Set 0 — 全局 Bindless 资源表（shader 可见，所有 draw 共享）
├─ binding=0: uniform texture2D   g_Textures[]      [65536]  — 所有贴图
├─ binding=1: uniform sampler     g_Samplers[]      [2048]   — 所有采样器
├─ binding=2: uniform texture2D   g_StorageImages[] [65536]  — RWTexture
├─ binding=3: uniform textureBuffer g_TexelBuffers[] [65536] — Buffer 视图（可选）
│
Set 1 — Per-Pass 数据（相机、光照、帧参数，每 pass 绑一次）
├─ binding=0: uniform PassUBO { mat4 viewProj; vec3 camPos; ... }
├─ binding=1: uniform LightData { ... }  — 或合并到 PassUBO
│
Set 2 — Per-Draw 数据（Material 参数，push constants 替代）
├─ binding=0: uniform MaterialUBO { vec4 baseColor; float roughness; ... }
│   （可选——如果 material 参数少，直接 push constants 就能覆盖，省掉 set 2）
```

**Set 2 可省略的条件**：每个 material 的参数 ≤ 128 bytes（即 ≤ push constants 上限，Vulkan 保证 128B，大多数 GPU 支持 256B）。SHZK 初期完全可以用 push constants 传 material 参数 + bindless 索引，走 Set 0 + Set 1 + PushConstants 三层。

#### RHI 层：BindlessTable

新类 `RHIBindlessTable`（RHI 抽象），`VulkanRHIBindlessTable`（Vulkan 实现）：

```cpp
class RHIBindlessTable : public RHIResource {
public:
    // 注册资源到全局表，返回 bindless index
    virtual uint32_t RegisterTexture(std::shared_ptr<RHITextureView> textureView) = 0;
    virtual uint32_t RegisterSampler(std::shared_ptr<RHISampler> sampler) = 0;
    virtual uint32_t RegisterStorageImage(std::shared_ptr<RHITextureView> textureView) = 0;

    // 释放 index（归还 free list）
    virtual void UnregisterTexture(uint32_t index) = 0;
    virtual void UnregisterSampler(uint32_t index) = 0;

    // 获取 descriptor sets（供 CommandContext 绑定）
    virtual VkDescriptorSet GetSet0() = 0;  // bindless table
    virtual VkDescriptorSet GetSet1() = 0;  // per-pass
...
- **Free-list index 分配** — 用 `std::vector<uint32_t> m_freeIndices` + `uint32_t m_nextIndex`。Register 时优先从 free list 取，否则 `m_nextIndex++`。Unregister 归还到 free list。
- **延迟更新** — Register/Unregister 不立即写 VkDescriptorSet。它们标记 dirty，`FlushPendingUpdates()` 时统一 `vkUpdateDescriptorSets`。这避免了 command buffer 录制期间的 VK 调用。
- **绑定时机** — `FlushPendingUpdates` 在每帧 `BeginCommand` 之后、第一次 draw 之前调用。更新完 set 0 后，`vkCmdBindDescriptorSets(set=0, ...)` 一次就覆盖全帧。
- **DescriptorSetLayout 创建** — 使用 `VK_DESCRIPTOR_SET_LAYOUT_CREATE_UPDATE_AFTER_BIND_POOL_BIT`，binding 设置 `VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT | VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT | VK_DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT_BIT`。

#### RenderResource 层的改变

```
传统（per-material descriptor set）:
  GpuMaterial → RHIDescriptorSet → [texture0: ???, texture1: ???, sampler: ???]

Bindless:
  GpuTexture  → uint32_t m_bindlessIndex   (在 GpuTexture::Init 时注册到 BindlessTable)
  GpuSampler  → uint32_t m_bindlessIndex
  GpuMaterial → DrawCommand { uint baseColorTexIdx; uint normalTexIdx; uint samplerIdx; ... }
```

`GpuMaterial` 不再持有 `RHIDescriptorSet`。它只持有 bindless 索引 + material 参数（baseColor, roughness 等），draw 时通过 push constants 传给 shader。
...
// Set 1 — Per-Pass
layout(set=1, binding=0) uniform PassUBO { mat4 viewProj; vec3 camPos; } g_Pass;

// Push Constants — Per-Draw Material
layout(push_constant) uniform MaterialParams {
    uint  baseColorTexIdx;    // index into g_Textures
    uint  normalTexIdx;
    uint  samplerIdx;         // index into g_Samplers
...
void main() {
    vec4 albedo = texture(sampler2D(g_Textures[g_Material.baseColorTexIdx],
                                   g_Samplers[g_Material.samplerIdx]), uv);
    // ...
}
```
...
```
每帧 BeginCommand:
  1. vkCmdBindDescriptorSets(set=0, set=1)     ← 一次绑定，全帧不变
  2. BindlessTable::FlushPendingUpdates()       ← 将新注册的纹理/sampler 写进 set 0

每个 Draw:
  3. vkCmdPushConstants(materialIndices + materialParams)
  4. vkCmdDrawIndexed(...)
...
**关键**：全帧只有 1 次 descriptor set bind（在帧开头），之后每个 draw 只 push 几十个字节的常量。对比传统方式每个不同 material 都要 bind descriptor set，性能提升明显——尤其在大规模场景中。

#### 实现优先级

1. **RHI 层**：`RHIBindlessTable` 抽象 + `VulkanRHIBindlessTable` 实现（descriptor set layout, pool, set allocation, register/unregister, flush）
2. **联接 RenderResource**：`GpuTexture::Init()` 中调 `BindlessTable::RegisterTexture(this->m_textureView)` → 存 `m_bindlessIndex`
3. **Shader 端**：所有 shader 统一使用 `g_Textures[index]` + `g_Samplers[index]` 的 bindless 语法
4. **帧循环**：`RenderSystem::Tick()` 中在 `BeginCommand` 后绑定 Set 0 + Set 1

不需要等所有 material 系统做完再设计 bindless——bindless 是基础设施层的概念，从第一天就应该按 bindless 设计 shader 接口和 Resource 注册流程。
