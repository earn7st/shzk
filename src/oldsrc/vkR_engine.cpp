...
#include "vkR_images.h"
#include "vkR_pipelines.h"
#include "vkR_types.h"
...
void VulkanEngine::init()
{
    ...
    init_swapchain_vkb(_windowExtent.width, _windowExtent.height);
...
void VulkanEngine::draw()
{
    ...
    //begin the command b   uffer recording. We will use this command buffer exactly once, so we want to let vulkan know that
...
// Draw Geometry: Triangle / Rectangle / Custom Mesh
...
void VulkanEngine::init_vulkan_vkb()
{

    // Initialize Instance
    vkb::InstanceBuilder builder;

    auto inst_ret = builder.set_app_name("vkR")
...
void VulkanEngine::init_vulkan()
{
    ...
    VK_CHECK(vkCreateInstance(&instanceCI, nullptr, &_instance));

}

void VulkanEngine::init_swapchain_vkb(uint32_t width, uint32_t height)
{
...
void VulkanEngine::destroy_mesh_asset(std::shared_ptr<MeshAsset>& meshAsset)
{
    destroy_buffer(meshAsset->meshBuffers.vertexBuffer);
    destroy_buffer(meshAsset->meshBuffers.indexBuffer);
}

void VulkanEngine::init_commands_vkb()
{
...
void VulkanEngine::init_descriptors()
{
    ...
    uint32_t maxSets = 10;
    globalDescriptorAllocator.init(_device, maxSets, sizes);
    ...
    // Init the Descriptor Layout for Single Image Texture
    {
        DescriptorLayoutBuilder builder;
    ...
    /*
     * Before Abstraction for DescriptorSets
...
void VulkanEngine::init_background_pipelines()
{
    ...
    if (!vkutil::load_shader_module("./shaders/gradient_color.comp.spv", _device, &computeGradientShader))
    {
